...
class LoremIpsumRequest(BaseModel):
    ...
    min_words: int = Field(
        5, ge=1, description="Mínimo de palabras por párrafo")
    max_words: int = Field(20, ge=5, le=lorem_settings.MAX_WORDS_PER_PARAGRAPH,
                           description="Máximo de palabras por párrafo")
    start_with_lorem: bool = Field(
        True, description="Si debe comenzar con 'Lorem ipsum dolor sit amet'")
    ...
    def validate_word_range(self):
        ...
        if self.min_words > self.max_words:
            raise ValueError(
                f"El mínimo de palabras ({self.min_words}) no puede ser mayor que el máximo ({self.max_words})")
        return True
    ...
    def validate_paragraphs(cls, v):
        if v > lorem_settings.MAX_PARAGRAPHS:
            raise ValueError(
                f"El número máximo de párrafos permitido es {lorem_settings.MAX_PARAGRAPHS}")
        return v
    ...
    def validate_max_words(cls, v):
        if v > lorem_settings.MAX_WORDS_PER_PARAGRAPH:
            raise ValueError(
                f"El número máximo de palabras por párrafo permitido es {lorem_settings.MAX_WORDS_PER_PARAGRAPH}")
        return v
...
class LoremIpsumResponse(BaseModel):
    ...
    total_words: int = Field(...,
                             description="Número total de palabras generadas")
